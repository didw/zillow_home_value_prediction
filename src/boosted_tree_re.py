...
df_train.fillna(98765,inplace = True)

x_train = df_train.drop(['parcelid', 'logerror', 'transactiondate', 'propertyzoningdesc', 
                         'propertycountylandusecode', 'fireplacecnt', 'fireplaceflag'], axis=1)
#x_train['Ratio_1'] = x_train['taxvaluedollarcnt']/x_train['taxamount']
print(max(df_train['logerror'].values), min(df_train['logerror'].values))
y_train = logerror_to_relativeerror(df_train['logerror'].values)
...
clf = lgb.train(params, d_train, 430, categorical_feature=[0,1,2,4,5,6,7,9,22,26,33,34,37,38,40,41,45,49,51,53])#31 error
...
p_test = relativeerror_to_logerror(clf.predict(x_test))
...
properties['nancnt'] = properties.isnull().sum()
##### PROCESS DATA FOR XGBOOST
print( "\nProcessing data for XGBoost ...")
for c in properties.columns:
    properties[c]=properties[c].fillna(98765)
...
x_train=train_df.drop(['parcelid', 'logerror','transactiondate'], axis=1)
y_train = logerror_to_relativeerror(train_df["logerror"].values.astype(np.float32))
...
xgb_pred1 = relativeerror_to_logerror(model.predict(dtest))
...
xgb_pred2 = relativeerror_to_logerror(model.predict(dtest))
...
y = logerror_to_relativeerror(train['logerror'].values)
...
print(MAE(y, relativeerror_to_logerror(reg.predict(train))))
train = [];  y = [] #memory
...
for i in range(len(test_dates)):
    ...
    pred = OLS_WEIGHT*relativeerror_to_logerror(reg.predict(get_features(test))) + (1-OLS_WEIGHT)*pred0
...
submission.to_csv('sub_re{}.csv'.format(datetime.now().strftime('%Y%m%d_%H%M%S')), index=False)
print( "\nFinished ...")
